...
def _select(country_code):
    ...
    candidates = (
        ...
        (
            "em",
            providers.electricity_maps.enabled and providers.electricity_maps.token,
            ElectricityMapsProvider,
        ),
        (
            "co2signal",
            providers.co2signal.enabled and providers.co2signal.token,
            Co2SignalProvider,
        ),
        (
            "watttime",
            providers.watttime.enabled
            and providers.watttime.username
            and providers.watttime.password,
            WattTimeProvider,
        ),
        (
            "entsoe",
            providers.entsoe.enabled
            and providers.entsoe.token
            and entsoe_cc in ENTSOE_DOMAIN,
            EntsoeProvider,
        ),
    )

    enabled = [(name, cls) for name, configured, cls in candidates if configured]
    if len(enabled) > 1:
        raise ProviderError(
